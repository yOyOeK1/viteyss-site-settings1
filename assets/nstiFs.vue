...
export default{
    ...
    data(){
        ...
        let fileName = timestampToNiceTime_DomSafe();
        let extTo = '.js';
        ...
        if( typeof this.data != 'string' ){
            ...
            if( 'fileName' in this.data && this.data['fileName'].length>1 ) fileName = this.data['fileName'];
...
observer START for [html@local]`*/,
    ...
    methods:{
        ...
        async saveTo_iFs_withData( onDoneRes, dirName, fileName, dataTo, overrite = false ){
            await iFs.createDirectory(dirName);
            let path = `${dirName}/${fileName}`;
            const fileExists = await iFs.exists( path );
            if( fileExists == true && !overrite ){
                
                let  userConfirmation = confirm(`File: [${path}] exists. Do you want to overrite?`);
                if (userConfirmation) {
                    // yes
                    await iFs.writeFile( path, dataTo );
                    onDoneRes('done');
                } else {
                    // no - abord
                    onDoneRes( 'abord' );
